...
public class SalaryServiceImpl implements SalaryService {
	...
	@Transactional
	// —творенн€ зарплати - берем юзера, дивимс€ чи були в нього перерви
	// персональн≥,
	// €кшо були, то в≥дн≥маЇм в≥д загального роб„асу його перерви отримуЇмо
	// к-ть годин.берем його роль, дивимс€ на рейтинг,
	// значенн€ рейтинга множимо на значенн€ зарплати(залежно в≥д реальнов≥дпрац
	// годин), передаЇм це все SalaryDTO.
	public List<SalaryDTO> createSalary(long userId, Date from, Date to) {
		...
		for (Event event : user.getEvents()) {
			...
			if (from.before(event.getSchedule().getDate())
				...
				// €к≥йсь тип ≥венту Personal Break, то значить дн≥ були
				// робоч≥ми
